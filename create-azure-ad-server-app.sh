...
export RBAC_SERVER_APP_NAME="$1"
export RBAC_SERVER_APP_URL="http://$RBAC_SERVER_APP_NAME"

export GENERATED_SECRET="$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)"
export RBAC_SERVER_APP_SECRET=${RBAC_SERVER_APP_SECRET:-$GENERATED_SECRET}
...
az ad app create --display-name ${RBAC_SERVER_APP_NAME} \
    ...
    --required-resource-accesses @azure-server-manifest.json
...
echo "
export TF_VAR_aks_rbac_server_app_id="${RBAC_SERVER_APP_ID}"
export TF_VAR_aks_rbac_server_app_secret="${RBAC_SERVER_APP_SECRET}"
export TF_VAR_tenant_id="${DETECTED_TENANT_ID}"
